...
#[allow(unsafe_code, reason = "JNI requires stable exported symbol names")]
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_mantle_internal_MantleNative_loadBandcampItem<'local>(
    mut env: EnvUnowned<'local>,
    _class: JClass<'local>,
    source: JObject<'local>,
    reference: JObject<'local>,
) -> JObject<'local> {
    env.with_env(|env| load_bridge::load_bandcamp_item(env, &source, &reference))
        .resolve::<ThrowRuntimeExAndDefault>()
}

#[allow(unsafe_code, reason = "JNI requires stable exported symbol names")]
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_mantle_internal_MantleNative_loadBeamItem<'local>(
    mut env: EnvUnowned<'local>,
    _class: JClass<'local>,
    source: JObject<'local>,
    reference: JObject<'local>,
) -> JObject<'local> {
    env.with_env(|env| load_bridge::load_beam_item(env, &source, &reference))
        .resolve::<ThrowRuntimeExAndDefault>()
}

#[allow(unsafe_code, reason = "JNI requires stable exported symbol names")]
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_mantle_internal_MantleNative_loadGetyarnItem<'local>(
    mut env: EnvUnowned<'local>,
    _class: JClass<'local>,
    source: JObject<'local>,
    reference: JObject<'local>,
) -> JObject<'local> {
    env.with_env(|env| load_bridge::load_getyarn_item(env, &source, &reference))
        .resolve::<ThrowRuntimeExAndDefault>()
}

#[allow(unsafe_code, reason = "JNI requires stable exported symbol names")]
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_mantle_internal_MantleNative_loadHttpItem<'local>(
    mut env: EnvUnowned<'local>,
    _class: JClass<'local>,
    source: JObject<'local>,
    reference: JObject<'local>,
) -> JObject<'local> {
    env.with_env(|env| load_bridge::load_http_item(env, &source, &reference))
        .resolve::<ThrowRuntimeExAndDefault>()
}
...
#[allow(unsafe_code, reason = "JNI requires stable exported symbol names")]
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_mantle_internal_MantleNative_loadYoutubeItem<'local>(
    mut env: EnvUnowned<'local>,
    _class: JClass<'local>,
    source: JObject<'local>,
    reference: JObject<'local>,
) -> JObject<'local> {
    env.with_env(|env| load_bridge::load_youtube_item(env, &source, &reference))
        .resolve::<ThrowRuntimeExAndDefault>()
}
...
#[allow(unsafe_code, reason = "JNI requires stable exported symbol names")]
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_mantle_internal_MantleNative_processBandcampTrack<'local>(
    mut env: EnvUnowned<'local>,
    _class: JClass<'local>,
    track: JObject<'local>,
    executor: JObject<'local>,
) {
    env.with_env(|env| playback_bridge::process_bandcamp_track(env, &track, &executor))
        .resolve::<ThrowRuntimeExAndDefault>();
}

#[allow(unsafe_code, reason = "JNI requires stable exported symbol names")]
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_mantle_internal_MantleNative_processBeamTrack<'local>(
    mut env: EnvUnowned<'local>,
    _class: JClass<'local>,
    track: JObject<'local>,
    executor: JObject<'local>,
) {
    env.with_env(|env| playback_bridge::process_beam_track(env, &track, &executor))
        .resolve::<ThrowRuntimeExAndDefault>();
}
...
#[allow(unsafe_code, reason = "JNI requires stable exported symbol names")]
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_mantle_internal_MantleNative_processHttpTrack<'local>(
    mut env: EnvUnowned<'local>,
    _class: JClass<'local>,
    track: JObject<'local>,
    executor: JObject<'local>,
) {
    env.with_env(|env| playback_bridge::process_http_track(env, &track, &executor))
        .resolve::<ThrowRuntimeExAndDefault>();
}
...
#[allow(unsafe_code, reason = "JNI requires stable exported symbol names")]
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_mantle_internal_MantleNative_processYandexMusicTrack<'local>(
    mut env: EnvUnowned<'local>,
    _class: JClass<'local>,
    track: JObject<'local>,
    executor: JObject<'local>,
) {
    env.with_env(|env| playback_bridge::process_yandex_music_track(env, &track, &executor))
        .resolve::<ThrowRuntimeExAndDefault>();
}
