...
jq --exit-status '
  ...
  .closure.local_exact_version_audits == 140 and
  .closure.safe_to_deploy_audits == 120 and
  .closure.safe_to_run_audits == 20 and
  .closure.remaining_exemptions == 0 and
  .closure.fully_audited_packages == 170 and
  (.closure.canonical_tuple_sha256 | test("^[0-9a-f]{64}$")) and
  .review_scope.package_files == 7603 and .review_scope.package_lines == 4009282 and
  .review_scope.rust_files == 5312 and
  .review_scope.packages_with_unsafe_lexical_surface == 84 and
...
[[ "$(wc -l <"$tuples" | tr -d ' ')" == 140 ]]
[[ "$(awk -F '\t' '$3 == "safe-to-deploy" {count += 1} END {print count + 0}' "$tuples")" == 120 ]]
...
[[ "$(awk '/^who = "OpenAI Codex for Mantle <noreply@openai.com>"$/ {count += 1} END {print count + 0}' "$AUDITS")" == 140 ]]
...
jq --exit-status '
  ...
  .audit_graph.local_exact_version_audits == 140 and
  .audit_graph.safe_to_deploy_audits == 120 and
  .audit_graph.safe_to_run_audits == 20 and
  .audit_graph.config_exemptions == 0 and
  .audit_graph.fully_audited_packages == 170 and
...
jq --exit-status '.conclusion == "success" and (.vetted_fully | length) == 170 and (.vetted_with_exemptions | length) == 0' "$VET_RESULT" >/dev/null
...
jq --exit-status '.conclusion == "success" and (.vetted_fully | length) == 170 and (.vetted_with_exemptions | length) == 0' "$live_vet" >/dev/null

printf 'Publication Cargo Vet closure passed: 170 fully audited packages and zero exemptions.\n'
