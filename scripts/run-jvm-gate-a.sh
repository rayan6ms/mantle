...
cargo run --locked -q -p mantle-jvm-gate -- write-simple-http-client-connection-manager-consumer \
...
cargo run --locked -q -p mantle-jvm-gate -- write-http-configurable-consumer \
  --output "$WORK/GateHttpConfigurable.java"
...
java -Xverify:all \
  -cp "$REFERENCE_PROVIDER_TOOLS_CLASSPATH" GateRingBufferMath >"$WORK/ring-buffer-math-reference.txt"
java -Xverify:all \
  -cp "$GATE_CLASSPATH$classpath_separator$REFERENCE_PROVIDER_TOOLS_CLASSPATH" \
  GateRingBufferMath >"$WORK/ring-buffer-math-candidate.txt"
cmp "$WORK/ring-buffer-math-reference.txt" "$WORK/ring-buffer-math-candidate.txt"
grep --fixed-strings \
...
java -Xverify:all \
  -cp "$REFERENCE_PROVIDER_TOOLS_CLASSPATH" GateSimpleHttpClientConnectionManager \
  >"$WORK/simple-http-client-connection-manager-reference.txt"
java -Xverify:all \
  -cp "$GATE_CLASSPATH$classpath_separator$REFERENCE_PROVIDER_TOOLS_CLASSPATH" \
  GateSimpleHttpClientConnectionManager \
  >"$WORK/simple-http-client-connection-manager-candidate.txt"
cmp "$WORK/simple-http-client-connection-manager-reference.txt" \
  "$WORK/simple-http-client-connection-manager-candidate.txt"
grep --fixed-strings \
  ...
  "$WORK/simple-http-client-connection-manager-candidate.txt" >/dev/null
java -Xverify:all \
  -cp "$REFERENCE_PROVIDER_TOOLS_CLASSPATH" GateExtendedHttpClientBuilder \
  >"$WORK/extended-http-client-builder-reference.txt"
java -Xverify:all \
  -cp "$GATE_CLASSPATH$classpath_separator$REFERENCE_PROVIDER_TOOLS_CLASSPATH" \
  GateExtendedHttpClientBuilder >"$WORK/extended-http-client-builder-candidate.txt"
cmp "$WORK/extended-http-client-builder-reference.txt" \
  "$WORK/extended-http-client-builder-candidate.txt"
grep --fixed-strings \
...
java -Xverify:all \
  -cp "$REFERENCE_PROVIDER_TOOLS_CLASSPATH" GateBitStreamReader \
  >"$WORK/bit-stream-reader-reference.txt"
java -Xverify:all \
  -cp "$GATE_CLASSPATH$classpath_separator$REFERENCE_PROVIDER_TOOLS_CLASSPATH" \
  GateBitStreamReader >"$WORK/bit-stream-reader-candidate.txt"
cmp "$WORK/bit-stream-reader-reference.txt" "$WORK/bit-stream-reader-candidate.txt"
grep --fixed-strings \
...
java -Xverify:all \
  -cp "$REFERENCE_PROVIDER_TOOLS_CLASSPATH" GateByteBufferOutputStream \
  >"$WORK/byte-buffer-output-stream-reference.txt"
java -Xverify:all \
  -cp "$GATE_CLASSPATH$classpath_separator$REFERENCE_PROVIDER_TOOLS_CLASSPATH" \
  GateByteBufferOutputStream >"$WORK/byte-buffer-output-stream-candidate.txt"
cmp "$WORK/byte-buffer-output-stream-reference.txt" \
  "$WORK/byte-buffer-output-stream-candidate.txt"
grep --fixed-strings \
...
java -Xverify:all \
  -cp "$REFERENCE_PROVIDER_TOOLS_CLASSPATH" GateHttpConfigurable \
  >"$WORK/http-configurable-reference.txt"
java -Xverify:all \
  -cp "$GATE_CLASSPATH$classpath_separator$REFERENCE_PROVIDER_TOOLS_CLASSPATH" \
  GateHttpConfigurable >"$WORK/http-configurable-candidate.txt"
cmp "$WORK/http-configurable-reference.txt" "$WORK/http-configurable-candidate.txt"
grep --fixed-strings \
  'contracts=caller-implementation,dispatch-order,function-identity,consumer-identity,null-forwarding,failure-identity,generics,public-abstract-interface,no-defaults,reflection' \
  "$WORK/http-configurable-candidate.txt" >/dev/null
java -Xverify:all \
  -cp "$REFERENCE_PROVIDER_TOOLS_CLASSPATH" GateHttpContextFilter \
  >"$WORK/http-context-filter-reference.txt"
java -Xverify:all \
  -cp "$GATE_CLASSPATH$classpath_separator$REFERENCE_PROVIDER_TOOLS_CLASSPATH" \
  GateHttpContextFilter >"$WORK/http-context-filter-candidate.txt"
cmp "$WORK/http-context-filter-reference.txt" "$WORK/http-context-filter-candidate.txt"
grep --fixed-strings \
