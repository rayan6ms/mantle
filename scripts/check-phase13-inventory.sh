...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[2].classified_symbols == 194 and
  .cohorts[2].remaining_symbols == 25 and
  (.cohorts[2].completed_slices | length) == 29 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1427 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1282 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "audio-chunk-decoder-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '207 reference classes / 1,439 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,427 classified symbols and 1,335 unassessed symbols.\n'
