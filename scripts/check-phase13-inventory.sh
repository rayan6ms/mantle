...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1443 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1298 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "pcm-chunk-decoder-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '211 reference classes / 1,455 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
