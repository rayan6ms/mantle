...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[4].classified_symbols == 124 and
  .cohorts[4].remaining_symbols == 267 and
  (.cohorts[4].completed_slices | length) == 18 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 2400 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 2251 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "abstract-http-context-filter-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '345 reference classes / 2,393 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 2,400 classified symbols and 362 unassessed symbols.\n'
