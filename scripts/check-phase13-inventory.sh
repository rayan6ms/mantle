...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[4].classified_symbols == 29 and
  .cohorts[4].remaining_symbols == 362 and
  (.cohorts[4].completed_slices | length) == 4 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 2305 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 2156 and
  ([$classifications.symbols[] |
    select(.binary_name ==
  ...
  .phase_entry.next_slice == "exception-tools-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '333 reference classes / 2,307 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 2,305 classified symbols and 457 unassessed symbols.\n'
