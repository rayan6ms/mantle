...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 193 and
  .cohorts[3].remaining_symbols == 631 and
  (.cohorts[3].completed_slices | length) == 29 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1645 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1500 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "matroska-aac-track-consumer-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '243 reference classes / 1,657 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,645 classified symbols and 1,117 unassessed symbols.\n'
