...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 101 and
  .cohorts[3].remaining_symbols == 723 and
  (.cohorts[3].completed_slices | length) == 14 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1553 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1408 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "opus-packet-router-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '227 reference classes / 1,565 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,553 classified symbols and 1,209 unassessed symbols.\n'
