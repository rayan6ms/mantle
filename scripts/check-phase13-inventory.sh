...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 76 and
  .cohorts[1].remaining_symbols == 622 and
  (.cohorts[1].completed_slices | length) == 10 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 611 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 598 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "default-sound-cloud-data-reader-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '76 reference classes / 640 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 611 classified symbols and 2,151 unassessed symbols.\n'
