...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 538 and
  .cohorts[1].remaining_symbols == 160 and
  (.cohorts[1].completed_slices | length) == 76 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1073 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 971 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "C_SEMANTIC")] | length) == 96 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "youtube-persistent-http-stream-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '147 reference classes / 1,102 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,073 classified symbols and 1,689 unassessed symbols.\n'
