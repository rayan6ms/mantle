...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 132 and
  .cohorts[3].remaining_symbols == 692 and
  (.cohorts[3].completed_slices | length) == 20 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1584 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1439 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "flac-seek-point-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '233 reference classes / 1,596 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,584 classified symbols and 1,178 unassessed symbols.\n'
