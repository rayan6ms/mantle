...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[2].classified_symbols == 62 and
  .cohorts[2].remaining_symbols == 157 and
  (.cohorts[2].completed_slices | length) == 12 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1295 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1160 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "pcm-format-contracts" and
...
printf 'Phase 13 inventory tracks 1,295 classified symbols and 1,467 unassessed symbols.\n'
