...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 151 and
  .cohorts[1].remaining_symbols == 547 and
  (.cohorts[1].completed_slices | length) == 18 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 686 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 668 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "sound-cloud-data-loader-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '84 reference classes / 715 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 686 classified symbols and 2,076 unassessed symbols.\n'
