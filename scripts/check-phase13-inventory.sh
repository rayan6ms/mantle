...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 519 and
  .cohorts[3].remaining_symbols == 305 and
  (.cohorts[3].completed_slices | length) == 60 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1971 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1824 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "mpeg-file-track-provider-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '278 reference classes / 1,976 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,971 classified symbols and 791 unassessed symbols.\n'
