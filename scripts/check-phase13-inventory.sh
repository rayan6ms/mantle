...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 39 and
  .cohorts[3].remaining_symbols == 785 and
  (.cohorts[3].completed_slices | length) == 4 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1491 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1346 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "media-container-detection-result-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '217 reference classes / 1,503 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,491 classified symbols and 1,271 unassessed symbols.\n'
