...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 151 and
  .cohorts[3].remaining_symbols == 673 and
  (.cohorts[3].completed_slices | length) == 22 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1603 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1458 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "flac-track-info-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '235 reference classes / 1,615 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,603 classified symbols and 1,159 unassessed symbols.\n'
