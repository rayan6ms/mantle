...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 555 and
  .cohorts[1].remaining_symbols == 143 and
  (.cohorts[1].completed_slices | length) == 80 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1090 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 982 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "youtube-search-provider-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '151 reference classes / 1,119 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,090 classified symbols and 1,672 unassessed symbols.\n'
