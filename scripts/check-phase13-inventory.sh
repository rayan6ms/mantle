...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 784 and
  .cohorts[3].remaining_symbols == 40 and
  (.cohorts[3].completed_slices | length) == 103 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 2236 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 2087 and
  ([$classifications.symbols[] |
    select(.binary_name ==
  ...
  .phase_entry.next_slice == "pls-playlist-container-probe-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '322 reference classes / 2,238 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 2,236 classified symbols and 526 unassessed symbols.\n'
