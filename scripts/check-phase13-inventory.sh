...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 2155 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 2006 and
  ([$classifications.symbols[] |
    select(.binary_name ==
  ...
  .phase_entry.next_slice == "ogg-flac-codec-handler-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '307 reference classes / 2,157 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 2,155 classified symbols and 607 unassessed symbols.\n'
