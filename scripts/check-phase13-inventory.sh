...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 120 and
  .cohorts[3].remaining_symbols == 704 and
  (.cohorts[3].completed_slices | length) == 18 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1572 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1427 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "flac-metadata-header-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '231 reference classes / 1,584 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,572 classified symbols and 1,190 unassessed symbols.\n'
