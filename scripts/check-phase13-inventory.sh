...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 611 and
  .cohorts[1].remaining_symbols == 87 and
  (.cohorts[1].completed_slices | length) == 91 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1146 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1029 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "legacy-stream-map-formats-extractor-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '162 reference classes / 1,175 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,146 classified symbols and 1,616 unassessed symbols.\n'
