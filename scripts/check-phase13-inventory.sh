...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 15 and
  .cohorts[1].remaining_symbols == 683 and
  (.cohorts[1].completed_slices | length) == 2 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 550 and
  ...
  .phase_entry.next_slice == "probing-audio-source-manager-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '68 reference classes / 579 symbols' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 550 classified symbols and 2,212 unassessed symbols.\n'
