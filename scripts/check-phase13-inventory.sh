...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 647 and
  .cohorts[3].remaining_symbols == 177 and
  (.cohorts[3].completed_slices | length) == 81 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 2099 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1952 and
  ([$classifications.symbols[] |
    select(.binary_name ==
  ...
  .phase_entry.next_slice == "ogg-packet-input-stream-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '299 reference classes / 2,101 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 2,099 classified symbols and 663 unassessed symbols.\n'
