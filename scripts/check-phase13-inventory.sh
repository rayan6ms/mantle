...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 409 and
  .cohorts[3].remaining_symbols == 415 and
  (.cohorts[3].completed_slices | length) == 45 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1861 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1714 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "mp3-constant-rate-seeker-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '263 reference classes / 1,866 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,861 classified symbols and 901 unassessed symbols.\n'
