...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[2].classified_symbols == 98 and
  .cohorts[2].remaining_symbols == 121 and
  (.cohorts[2].completed_slices | length) == 19 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1331 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1190 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "to-short-audio-filter-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '194 reference classes / 1,358 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,331 classified symbols and 1,431 unassessed symbols.\n'
