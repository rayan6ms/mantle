...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 552 and
  .cohorts[1].remaining_symbols == 146 and
  (.cohorts[1].completed_slices | length) == 79 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1087 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 979 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "C_SEMANTIC")] | length) == 102 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "youtube-search-music-result-loader-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '150 reference classes / 1,116 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,087 classified symbols and 1,675 unassessed symbols.\n'
