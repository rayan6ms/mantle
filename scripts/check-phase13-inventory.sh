...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 364 and
  .cohorts[1].remaining_symbols == 334 and
  (.cohorts[1].completed_slices | length) == 53 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 899 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 831 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "yandex-music-search-result-loader-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '121 reference classes / 928 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 899 classified symbols and 1,863 unassessed symbols.\n'
