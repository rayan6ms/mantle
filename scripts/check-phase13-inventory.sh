...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 316 and
  .cohorts[1].remaining_symbols == 382 and
  (.cohorts[1].completed_slices | length) == 45 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 851 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 793 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "C_SEMANTIC")] | length) == 52 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "default-yandex-music-track-loader-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '113 reference classes / 880 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 851 classified symbols and 1,911 unassessed symbols.\n'
