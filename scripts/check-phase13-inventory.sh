...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[2].classified_symbols == 27 and
  .cohorts[2].remaining_symbols == 192 and
  (.cohorts[2].completed_slices | length) == 6 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1260 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1125 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "channel-count-pcm-audio-filter-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '181 reference classes / 1,289 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,260 classified symbols and 1,502 unassessed symbols.\n'
