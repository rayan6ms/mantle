...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[2].classified_symbols == 157 and
  .cohorts[2].remaining_symbols == 62 and
  (.cohorts[2].completed_slices | length) == 25 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1390 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1245 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "audio-player-input-stream-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '203 reference classes / 1,402 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,390 classified symbols and 1,372 unassessed symbols.\n'
