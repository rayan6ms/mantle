...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[2].classified_symbols == 23 and
  .cohorts[2].remaining_symbols == 196 and
  (.cohorts[2].completed_slices | length) == 5 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1256 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1121 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "buffering-post-processor-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '180 reference classes / 1,285 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,256 classified symbols and 1,506 unassessed symbols.\n'
