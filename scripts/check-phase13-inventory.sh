...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 460 and
  .cohorts[3].remaining_symbols == 364 and
  (.cohorts[3].completed_slices | length) == 52 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1912 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1765 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "mpeg-aac-track-consumer-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '270 reference classes / 1,917 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,912 classified symbols and 850 unassessed symbols.\n'
