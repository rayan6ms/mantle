...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 371 and
  .cohorts[1].remaining_symbols == 327 and
  (.cohorts[1].completed_slices | length) == 56 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 906 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 838 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "default-youtube-link-router-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '124 reference classes / 935 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 906 classified symbols and 1,856 unassessed symbols.\n'
