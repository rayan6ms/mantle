...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 116 and
  .cohorts[3].remaining_symbols == 708 and
  (.cohorts[3].completed_slices | length) == 17 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1568 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1423 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "flac-file-loader-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '230 reference classes / 1,580 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,568 classified symbols and 1,194 unassessed symbols.\n'
