...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 22 and
  .cohorts[1].remaining_symbols == 676 and
  (.cohorts[1].completed_slices | length) == 3 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 557 and
  ...
  .phase_entry.next_slice == "local-audio-source-manager-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '69 reference classes / 586 symbols' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 557 classified symbols and 2,205 unassessed symbols.\n'
