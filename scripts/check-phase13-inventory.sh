...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[0].classified_symbols == 375 and
  .cohorts[0].remaining_symbols == 160 and
  (.cohorts[0].completed_slices | length) == 27 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 375 and
  ...
  .phase_entry.next_slice == "audio-track-executor-contracts" and
...
printf 'Phase 13 inventory tracks 375 classified core-player-track symbols and 2,387 unassessed symbols.\n'
