...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[2].classified_symbols == 20 and
  .cohorts[2].remaining_symbols == 199 and
  (.cohorts[2].completed_slices | length) == 4 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1253 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1118 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "audio-post-processor-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '179 reference classes / 1,282 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,253 classified symbols and 1,509 unassessed symbols.\n'
