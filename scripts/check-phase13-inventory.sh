...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 608 and
  .cohorts[1].remaining_symbols == 90 and
  (.cohorts[1].completed_slices | length) == 90 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1143 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1026 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "legacy-dash-mpd-formats-extractor-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '161 reference classes / 1,172 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,143 classified symbols and 1,619 unassessed symbols.\n'
