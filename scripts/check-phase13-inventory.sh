...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 778 and
  .cohorts[3].remaining_symbols == 46 and
  (.cohorts[3].completed_slices | length) == 102 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 2230 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 2081 and
  ([$classifications.symbols[] |
    select(.binary_name ==
  ...
  .phase_entry.next_slice == "plain-playlist-container-probe-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '321 reference classes / 2,232 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 2,230 classified symbols and 532 unassessed symbols.\n'
