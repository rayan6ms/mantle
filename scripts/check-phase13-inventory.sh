...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 138 and
  .cohorts[3].remaining_symbols == 686 and
  (.cohorts[3].completed_slices | length) == 21 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1590 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1445 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "flac-stream-info-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '234 reference classes / 1,602 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,590 classified symbols and 1,172 unassessed symbols.\n'
