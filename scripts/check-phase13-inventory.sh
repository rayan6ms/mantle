...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 181 and
  .cohorts[1].remaining_symbols == 517 and
  (.cohorts[1].completed_slices | length) == 23 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 716 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 691 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "C_SEMANTIC")] | length) == 19 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "sound-cloud-m3u-audio-track-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '89 reference classes / 745 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 716 classified symbols and 2,046 unassessed symbols.\n'
