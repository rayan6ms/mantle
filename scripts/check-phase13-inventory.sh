...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 56 and
  .cohorts[3].remaining_symbols == 768 and
  (.cohorts[3].completed_slices | length) == 6 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1508 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1363 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "media-container-probe-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '219 reference classes / 1,520 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,508 classified symbols and 1,254 unassessed symbols.\n'
