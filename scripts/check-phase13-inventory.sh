...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 692 and
  .cohorts[3].remaining_symbols == 132 and
  (.cohorts[3].completed_slices | length) == 86 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 2144 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1995 and
  ([$classifications.symbols[] |
    select(.binary_name ==
  ...
  .phase_entry.next_slice == "ogg-track-blueprint-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '304 reference classes / 2,146 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 2,144 classified symbols and 618 unassessed symbols.\n'
