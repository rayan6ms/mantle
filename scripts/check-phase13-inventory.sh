...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 174 and
  .cohorts[1].remaining_symbols == 524 and
  (.cohorts[1].completed_slices | length) == 22 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 709 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 687 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "C_SEMANTIC")] | length) == 16 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "sound-cloud-http-context-filter-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '88 reference classes / 738 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 709 classified symbols and 2,053 unassessed symbols.\n'
