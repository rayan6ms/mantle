...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 61 and
  .cohorts[3].remaining_symbols == 763 and
  (.cohorts[3].completed_slices | length) == 7 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1513 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1368 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "media-container-registry-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '220 reference classes / 1,525 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,513 classified symbols and 1,249 unassessed symbols.\n'
