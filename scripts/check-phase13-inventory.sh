...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[4].classified_symbols == 25 and
  .cohorts[4].remaining_symbols == 366 and
  (.cohorts[4].completed_slices | length) == 3 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 2301 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 2152 and
  ([$classifications.symbols[] |
    select(.binary_name ==
  ...
  .phase_entry.next_slice == "decoded-exception-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '332 reference classes / 2,303 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 2,301 classified symbols and 461 unassessed symbols.\n'
