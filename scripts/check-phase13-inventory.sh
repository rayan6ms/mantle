...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 478 and
  .cohorts[3].remaining_symbols == 346 and
  (.cohorts[3].completed_slices | length) == 55 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1930 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1783 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "mpeg-file-loader-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '273 reference classes / 1,935 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,930 classified symbols and 832 unassessed symbols.\n'
