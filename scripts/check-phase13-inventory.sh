...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 552 and
  .cohorts[3].remaining_symbols == 272 and
  (.cohorts[3].completed_slices | length) == 66 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 2004 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1857 and
  ([$classifications.symbols[] |
    select(.binary_name ==
  ...
  .phase_entry.next_slice == "mpeg-versioned-section-handler-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '284 reference classes / 2,009 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 2,004 classified symbols and 758 unassessed symbols.\n'
