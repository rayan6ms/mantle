...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 429 and
  .cohorts[3].remaining_symbols == 395 and
  (.cohorts[3].completed_slices | length) == 48 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1881 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1734 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "mp3-seeker-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '266 reference classes / 1,886 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,881 classified symbols and 881 unassessed symbols.\n'
