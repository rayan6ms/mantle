...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[4].classified_symbols == 117 and
  .cohorts[4].remaining_symbols == 274 and
  (.cohorts[4].completed_slices | length) == 17 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 2393 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 2244 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "units-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '344 reference classes / 2,386 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 2,393 classified symbols and 369 unassessed symbols.\n'
