...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 614 and
  .cohorts[1].remaining_symbols == 84 and
  (.cohorts[1].completed_slices | length) == 92 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1149 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1032 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "C_SEMANTIC")] | length) == 111 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "offline-youtube-track-format-extractor-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '163 reference classes / 1,178 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,149 classified symbols and 1,613 unassessed symbols.\n'
