...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1438 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1293 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "opus-chunk-encoder-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '210 reference classes / 1,450 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
