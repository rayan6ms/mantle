...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 159 and
  .cohorts[3].remaining_symbols == 665 and
  (.cohorts[3].completed_slices | length) == 23 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1611 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1466 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "flac-track-info-builder-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '236 reference classes / 1,623 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,611 classified symbols and 1,151 unassessed symbols.\n'
