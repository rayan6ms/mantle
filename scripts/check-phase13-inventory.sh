...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 190 and
  .cohorts[3].remaining_symbols == 634 and
  (.cohorts[3].completed_slices | length) == 28 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1642 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1497 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "flac-sub-frame-reader-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '242 reference classes / 1,654 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,642 classified symbols and 1,120 unassessed symbols.\n'
