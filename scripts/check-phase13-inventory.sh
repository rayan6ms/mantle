...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 107 and
  .cohorts[3].remaining_symbols == 717 and
  (.cohorts[3].completed_slices | length) == 15 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1559 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1414 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "flac-audio-track-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '228 reference classes / 1,571 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,559 classified symbols and 1,203 unassessed symbols.\n'
