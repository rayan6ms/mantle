...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[4].classified_symbols == 252 and
  .cohorts[4].remaining_symbols == 139 and
  (.cohorts[4].completed_slices | length) == 39 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 2528 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 2379 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "extended-buffered-input-stream-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '372 reference classes / 2,524 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 2,528 classified symbols and 234 unassessed symbols.\n'
