...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 110 and
  .cohorts[3].remaining_symbols == 714 and
  (.cohorts[3].completed_slices | length) == 16 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1562 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1417 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "flac-container-probe-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '229 reference classes / 1,574 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,562 classified symbols and 1,200 unassessed symbols.\n'
