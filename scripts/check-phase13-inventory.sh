...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[4].classified_symbols == 43 and
  .cohorts[4].remaining_symbols == 348 and
  (.cohorts[4].completed_slices | length) == 5 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 2319 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 2170 and
  ([$classifications.symbols[] |
    select(.binary_name ==
  ...
  .phase_entry.next_slice == "friendly-exception-contracts" and
...
printf 'Phase 13 inventory tracks 2,319 classified symbols and 443 unassessed symbols.\n'
