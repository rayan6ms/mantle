...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 128 and
  .cohorts[3].remaining_symbols == 696 and
  (.cohorts[3].completed_slices | length) == 19 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1580 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1435 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "flac-metadata-reader-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '232 reference classes / 1,592 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,580 classified symbols and 1,182 unassessed symbols.\n'
