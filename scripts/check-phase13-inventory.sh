...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 366 and
  .cohorts[1].remaining_symbols == 332 and
  (.cohorts[1].completed_slices | length) == 54 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 901 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 833 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "yandex-music-track-loader-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '122 reference classes / 930 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 901 classified symbols and 1,861 unassessed symbols.\n'
