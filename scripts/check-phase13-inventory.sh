...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 368 and
  .cohorts[1].remaining_symbols == 330 and
  (.cohorts[1].completed_slices | length) == 55 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 903 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 835 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "yandex-music-utils-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '123 reference classes / 932 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 903 classified symbols and 1,859 unassessed symbols.\n'
