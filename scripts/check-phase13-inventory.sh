...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 51 and
  .cohorts[3].remaining_symbols == 773 and
  (.cohorts[3].completed_slices | length) == 5 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1503 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1358 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "media-container-hints-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '218 reference classes / 1,515 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,503 classified symbols and 1,259 unassessed symbols.\n'
