...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 623 and
  .cohorts[1].remaining_symbols == 75 and
  (.cohorts[1].completed_slices | length) == 95 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1158 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1041 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "bandcamp-audio-source-manager-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '166 reference classes / 1,187 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,158 classified symbols and 1,604 unassessed symbols.\n'
