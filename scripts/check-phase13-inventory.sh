...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[2].classified_symbols == 214 and
  .cohorts[2].remaining_symbols == 5 and
  (.cohorts[2].completed_slices | length) == 34 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1447 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1302 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "pcm-chunk-encoder-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '212 reference classes / 1,459 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,447 classified symbols and 1,315 unassessed symbols.\n'
