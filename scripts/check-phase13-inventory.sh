...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[2].classified_symbols == 154 and
  .cohorts[2].remaining_symbols == 65 and
  (.cohorts[2].completed_slices | length) == 24 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1387 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1242 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "audio-data-format-tools-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '202 reference classes / 1,399 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,387 classified symbols and 1,375 unassessed symbols.\n'
