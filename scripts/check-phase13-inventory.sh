...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 684 and
  .cohorts[3].remaining_symbols == 140 and
  (.cohorts[3].completed_slices | length) == 85 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 2136 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1987 and
  ([$classifications.symbols[] |
    select(.binary_name ==
  ...
  .phase_entry.next_slice == "ogg-stream-size-info-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '303 reference classes / 2,138 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 2,136 classified symbols and 626 unassessed symbols.\n'
