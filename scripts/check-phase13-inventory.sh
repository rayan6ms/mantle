...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[2].classified_symbols == 164 and
  .cohorts[2].remaining_symbols == 55 and
  (.cohorts[2].completed_slices | length) == 26 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1397 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1252 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "opus-audio-data-format-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '204 reference classes / 1,409 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,397 classified symbols and 1,365 unassessed symbols.\n'
