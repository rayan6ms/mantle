...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 319 and
  .cohorts[1].remaining_symbols == 379 and
  (.cohorts[1].completed_slices | length) == 46 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 854 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 794 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "C_SEMANTIC")] | length) == 54 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "default-yandex-search-provider-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '114 reference classes / 883 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 854 classified symbols and 1,908 unassessed symbols.\n'
