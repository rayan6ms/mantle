...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .existing_structural_slice.classes == 54 and
  .existing_structural_slice.symbols == 403 and
  .existing_structural_slice.internal_runtime_classes == 11 and
  (.existing_structural_slice.binary_names | length) == 54 and
  (.existing_structural_slice.binary_names | unique | length) == 54 and
  ...
  ([.existing_structural_slice.binary_names[] as $name |
    ...
    1 + (.fields | length) + (.methods | length)] | add) == 403 and
  ...
  .cohorts[3].classified_symbols == 261 and
  .cohorts[3].remaining_symbols == 563 and
  (.cohorts[3].completed_slices | length) == 37 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1713 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1566 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "matroska-cue-point-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '252 reference classes / 1,725 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,713 classified symbols and 1,049 unassessed symbols.\n'
