...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 447 and
  .cohorts[1].remaining_symbols == 251 and
  (.cohorts[1].completed_slices | length) == 65 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 982 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 893 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "C_SEMANTIC")] | length) == 83 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "youtube-audio-track-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '133 reference classes / 1,011 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 982 classified symbols and 1,780 unassessed symbols.\n'
