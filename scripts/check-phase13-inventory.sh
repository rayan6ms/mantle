...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 455 and
  .cohorts[3].remaining_symbols == 369 and
  (.cohorts[3].completed_slices | length) == 51 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1907 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1760 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "mp3-xing-seeker-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '269 reference classes / 1,912 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,907 classified symbols and 855 unassessed symbols.\n'
