...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .cohorts[1].classified_symbols == 73 and
  .cohorts[1].remaining_symbols == 625 and
  (.cohorts[1].completed_slices | length) == 9 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 608 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 595 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "default-sound-cloud-data-loader-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '75 reference classes / 637 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 608 classified symbols and 2,154 unassessed symbols.\n'
