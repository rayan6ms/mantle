...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" '
  ...
  .existing_structural_slice.classes == 41 and
  .existing_structural_slice.symbols == 329 and
  .existing_structural_slice.internal_runtime_classes == 11 and
  (.existing_structural_slice.binary_names | length) == 41 and
  (.existing_structural_slice.binary_names | unique | length) == 41 and
  ...
  ([.existing_structural_slice.binary_names[] as $name |
    ...
    1 + (.fields | length) + (.methods | length)] | add) == 329 and
  ...
  .cohorts[0].classified_symbols == 224 and
  .cohorts[0].remaining_symbols == 311 and
  (.cohorts[0].completed_slices | length) == 12 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 224 and
  ...
  .phase_entry.next_slice == "reference-mutable-audio-frame-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '41 reference classes / 329 symbols' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 224 classified core-player-track symbols and 2,538 unassessed symbols.\n'
