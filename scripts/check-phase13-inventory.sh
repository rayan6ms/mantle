...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 438 and
  .cohorts[3].remaining_symbols == 386 and
  (.cohorts[3].completed_slices | length) == 50 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1890 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1743 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "mp3-track-provider-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '268 reference classes / 1,895 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,890 classified symbols and 872 unassessed symbols.\n'
