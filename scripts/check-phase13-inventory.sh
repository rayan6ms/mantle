...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[4].classified_symbols == 257 and
  .cohorts[4].remaining_symbols == 134 and
  (.cohorts[4].completed_slices | length) == 40 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 2533 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 2384 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "greedy-input-stream-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '373 reference classes / 2,529 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 2,533 classified symbols and 229 unassessed symbols.\n'
