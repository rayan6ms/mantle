...
jq --exit-status --slurpfile inventory "$INVENTORY" --slurpfile ledger "$LEDGER" \
  ...
  .cohorts[3].classified_symbols == 433 and
  .cohorts[3].remaining_symbols == 391 and
  (.cohorts[3].completed_slices | length) == 49 and
  ...
  ([$classifications.symbols[] | select(.assessment == "CLASSIFIED")] | length) == 1885 and
  ([$classifications.symbols[] |
    select(.assessment == "CLASSIFIED" and .classification == "A_EXACT")] | length) == 1738 and
  ([$classifications.symbols[] |
  ...
  .phase_entry.next_slice == "mp3-stream-seeker-contracts" and
...
for required in \
  ...
  '2,762 symbols' \
  '267 reference classes / 1,890 symbols' \
  'C_SEMANTIC' \
  'D_LEGACY' \
  'core-player-track' \
  'Phase 12'; do
...
printf 'Phase 13 inventory tracks 1,885 classified symbols and 877 unassessed symbols.\n'
